...
class AI(object):
    ...
    def select_random_field(self):
        while True:
            ...
            if self.board[self.row_position][self.column_position] != \
                    Fieldtype.Fieldtype.Hit.value and self.board[self.row_position][
    ...
    def make_move(self):  # returns the x,y position which is chosen by the Algorithm
        ...
        if self.state == AIState.AIState.Searching:
            ...
            # top
            if self.dirs == DirectionList.DirectionList.Top and self.row_position > 0:
                self.temp_row_position = self.row_position - self.index
                if self.temp_row_position >= 0 and self.board[self.row_position - self.index][
                    self.column_position] != Fieldtype.Fieldtype.Miss:
            ...
            # bottom
            if self.dirs == DirectionList.DirectionList.Bottom and self.row_position < self.board_length - 1:
                self.temp_row_position = self.row_position + self.index
                if self.temp_row_position < self.board_length and self.board[self.row_position + self.index][
                    self.column_position] != Fieldtype.Fieldtype.Miss:
                    if self.fields[self.temp_row_position][self.column_position] == Fieldtype.Fieldtype.Ship.value:
                        self.board[self.temp_row_position][self.column_position] = Fieldtype.Fieldtype.Hit.value
                        self.ship_length += 1
            ...
            # left
            if self.dirs == DirectionList.DirectionList.Left and self.column_position > 0 and self.direction != \
            ...
            # right
            if self.dirs == DirectionList.DirectionList.Right and self.column_position < self.board_length - 1 and self.direction != Direction.Direction.Vertical:
                self.temp_column_position = self.column_position + self.index
                    ...
                    for tempx in range(x - 1, x + 2):
                        if 0 <= tempy < self.board_length:
                            if 0 <= tempx < self.board_length:
                                if self.board[tempy][tempx] != Fieldtype.Fieldtype.Hit.value and self.fields[tempy][
                                    tempx] != Fieldtype.Fieldtype.Ship.value:
